    ...
    <header className="fixed w-full top-0 z-50 bg-[#1E1E1E]/80 backdrop-blur-lg border-b border-gray-800">
      <nav className="container mx-auto px-6">
        <div className="flex h-16 items-center justify-between">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2 text-gray-200 hover:text-white transition-colors">
            ...
            <div className="flex items-center space-x-4 ml-4 relative">
              {state?.isAuthenticated ? (
                <div className="relative" >
                  ...
                  {dropdownOpen && (
                    <DropDown setDropDownOpen={() => setDropdownOpen(dropdownOpen)} dropdownOpen={dropdownOpen} state={state} dispatch={dispatch} />
                  )}
                </div>
              ) : (
                <Link
